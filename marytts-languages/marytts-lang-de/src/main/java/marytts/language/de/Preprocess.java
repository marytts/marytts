...
public class Preprocess extends MaryModule {
	...
	super("preprocessing");
	this.lexicon = null;
        this.rbnf = new RuleBasedNumberFormat(ULocale.GERMAN, RuleBasedNumberFormat.SPELLOUT);
        this.cardinalRule = "%spellout-numbering";
        this.ordinalRule = getOrdinalRuleName(rbnf);
        this.yearRule = getYearRuleName(rbnf);
        this.df = DateFormat.getDateInstance(DateFormat.LONG, ULocale.GERMAN);
        try {
	...
	this.description = "Token preprocessing class for german";
    }
    ...
    protected String expandDate(String date) throws ParseException {
        ...
        Date humanDate = df.getPatternInstance("MM.dd.yyyy", ULocale.GERMAN).parse(date);
