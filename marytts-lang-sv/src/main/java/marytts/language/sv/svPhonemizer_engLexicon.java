 ...
 * 
 * This file has been tweaked slightly from the original file JPhonemiser.java.
 * It is used in tandem with svPhonemizer.java to generate pronunciations for 
 * Swedish text including loan words and English terminology.
 *
 * @author Marc Schr&ouml;der, Sathish, tweaked by Erik Sterneberg
 */

public class svPhonemizer_engLexicon extends InternalModule
{
    ...
    /*
     * 	Needs en_GB voices to be installed
     */
    public svPhonemizer_engLexicon() throws IOException,  MaryConfigurationException{
        this("svPhonemizer_engLexicon", MaryDataType.PARTSOFSPEECH, MaryDataType.PHONEMES,
                MaryProperties.needFilename("sv.engAllophoneset"),
                MaryProperties.getFilename("sv.engUserdict"),
                MaryProperties.needFilename("sv.engLexicon"),
                MaryProperties.needFilename("sv.engLettertosound"));
    }
     ...
     * @throws Exception
     */
    public svPhonemizer_engLexicon(String componentName, 
    ...
    throws IOException, MaryConfigurationException
    {
        ...
        lts = new TrainedLTS(allophoneSet, new FileInputStream(ltsFilename));
    }
    ...
    public String phonemise(String text, String pos, StringBuilder g2pMethod)
    {
        ...
        //If not in English lexicon, don't use LTS rules. That method will be called later if necessary.
    ...
    /*
    public String phonemise_UseDicts(String text, String pos, StringBuilder g2pMethod){
        // First, try a simple userdict and lexicon lookup:

        String result = userdictLookup(text, pos);
        if (result != null) {
            g2pMethod.append("english userdict");
            return result;
        }
                
        result = lexiconLookup(text, pos);
        if (result != null) {
            g2pMethod.append("english lexicon");
            System.out.println("Found word in English lexicon. Pronunciation: " + result);
            return result;
        }
        ...
        if (!normalised.equals(text)) {
            ...
            if (result != null) {
                g2pMethod.append("english userdict");
                System.out.println("Found word in English user dictionary after normalizing exotic letters. Pronunciation: " + result);
                return result;
            }
            result = lexiconLookup(normalised, pos);
            if (result != null) {
                g2pMethod.append("english lexicon");
                System.out.println("Found word in English lexicon after normalizing exotic letters. Pronunciation: " + result);
                return result;
            }
        }
           
        // If the word was not in either the user dictionary or the lexicon, return null
        return null;
    }
      */  
    
    public String ltsLookup(String text){
        // Cannot find it in the lexicon -- apply letter-to-sound rules
        // to the normalised form

        String phones = lts.predictPronunciation(text);
        String result = lts.syllabify(phones);
        if (result != null) {            
            return result;
        }
