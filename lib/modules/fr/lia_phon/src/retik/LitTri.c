...
for (nb=1;fgets(ch,200,file);nb++)
 ...
 for(n=8;ch[n]!=' ';n++);
 ch[n]='\0';
 resu[nb].label=(char *)strdup(ch+8);
...
for (nb=1;nb<=NbClass;nb++)
 {
 class[nb].score=1./*(double)((double)class[nb].nbmots/(double)class[0].nbmots)*/;
 if (comm)
  printf("Classe : %s = %d  Total = %d  Score=%lf\n",
	class[nb].label,class[nb].nbmots,class[0].nbmots,class[nb].score);
 }
...
int n,re=1;
for(n=2;n<=NbClass;n++)
 if (class[n].score>class[re].score) re=n;
return re;
}
...
for (nb=1;nb<=NbClass;nb++)
 printf("Classe : %s  Score : %f\n",class[nb].label,class[nb].score);
}
