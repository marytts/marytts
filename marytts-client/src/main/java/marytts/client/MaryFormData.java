    ...
    public void toAudioFileFormatAndOutTypes(String info)
    {
        ...
        if (allTypes!=null)
        {
            ...
            { 
                ...
                if (typeName.equals("MP3")) isSupported = false; //MaryServerUtils.canCreateMP3();
                else if (typeName.equals("Vorbis")) isSupported = false; //MaryServerUtils.canCreateOgg();
                try {
        ...
        else
        {
            if (allVoices!=null && allVoices.size()>0 && inputDataTypes!=null && inputDataTypes.size()>0)
            {
                if (allVoices.elementAt(voiceSelected).isLimitedDomain()
                    ...
                    inputText = serverExampleTexts.get(inputDataTypes.get(inputTypeSelected).name()+" "+allVoices.elementAt(voiceSelected).getLocale().toString());
            } 
        }
        //
