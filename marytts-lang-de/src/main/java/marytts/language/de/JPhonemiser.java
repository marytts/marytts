...
public class JPhonemiser extends marytts.modules.JPhonemiser
{
        ...
        try {
            ...
            PrintWriter logUnknown = new PrintWriter(
                    new OutputStreamWriter(
                            new FileOutputStream(logUnknownFileName), "UTF-8"));
            //sort the words
            ...
            PrintWriter logEnglish = new PrintWriter(
                    new OutputStreamWriter(
                            new FileOutputStream(logEnglishFileName), "UTF-8"));
            //sort the words
    ...
    public String phonemise(String text, String pos, StringBuilder g2pMethod)
    {
        ...
        /**
        // Not found? Try a compound "analysis":
        result = compoundSearch(text);
        //logger.debug("Compound here: "+compoundSearch(text));
        if (result != null) {
        ...
        if (!normalised.equals(text)) {
            ...
            /**
            // Not found? Try a compound "analysis":
            result = compoundSearch(normalised);
            if (result != null) {
                g2pMethod.append("compound");
                return result;
            }**/
     ...
     * @param text the word to be transcribed.
     * @return the SAMPA transcription of text, or null if none was found.
     */
    /*
    private String compoundSearch(String text)
    {
        // Chop off longest possible prefixes and try to look them up
        // in the lexicon. Any part must have a minimum length of 3 characters.
        ...
        for (int i=text.length() - 3; i >= 3; i--) { //-3!!! >= 3!!!
            ...
            if (firstPhon != null) { // found a valid prefix
                ...
                restPhon = userdictLookup(rest);
                
                if (restPhon == null) restPhon = lexiconLookup(rest);
                
                // Or can the rest be analysed as a compound?
                if (restPhon == null) restPhon = compoundSearch(rest);
                
                // Or does it help if we cut off a Fuge?
                if (restPhon == null) {
                    String [] helper = fugeSearch(rest);
                    //hier scheint er nicht mehr reinzugehen
                    //logger.debug("fugeSearch(rest) is: " + fugeSearch(rest));
                    if (helper != null && helper.length == 2) {
                        fugePhon = helper[0];
                        String rest2 = helper[1];
                        assert fugePhon != null;
                        assert rest2 != null;
                        restPhon = userdictLookup(rest2);
                        if (restPhon == null) restPhon = lexiconLookup(rest2);
                        if (restPhon == null) restPhon = compoundSearch(rest2);
                    }
                }
                if (restPhon != null) // success!
                    return firstPhon + (fugePhon != null ? fugePhon : "") +
                        "-" + restPhon;
            }
        }
        return null;
   ...
   */
     ...
     * @return a two-item String array. First string is the transcription of the
    ...
    private String[] fugeSearch(String suffix)
    {
        String fugePhon = null;
        int fugeLength = 0;
        if (suffix.startsWith("es")) {
            fugePhon = "@s";
            fugeLength = 2;
            } else if (suffix.startsWith("en")) {
            fugePhon = "@n";
            fugeLength = 2;
