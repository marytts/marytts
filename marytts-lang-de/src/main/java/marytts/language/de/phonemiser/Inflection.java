...
public class Inflection
{
    ...
    public void determineEndings(Document doc) {
        ...
        while ((toInflect = (Element) ni.nextNode()) != null) {
            ...
            if (!toInflect.hasAttribute("sounds_like")) {
                logger.warn(
                    "Token `"
                        + MaryDomUtils.tokenText(toInflect)
            ...
            Set<String> endingClasses =
                new HashSet<String>(Arrays.asList(new String[] { "1", "2", "3", "4", "5", "6", "7", "8", "9" }));
            // Also need the determiner type:
            String detType = null;
            TreeWalker tw =
                ((DocumentTraversal) doc).createTreeWalker(doc, NodeFilter.SHOW_ELEMENT, new NameNodeFilter(MaryXML.TOKEN), true);
            ...
            if (!toInflect.getAttribute("syn_attach").equals("1")) {
                ...
                while (!foundStart && (t = (Element) tw.previousNode()) != null) {
                    ...
                    if (synAttach.equals("1")
                        || t.getAttribute("syn_phrase").equals("CNP")
            ...
            boolean haveSeenNoun = false;
                            while ((t = (Element) tw.nextNode()) != null
                                && !t.getAttribute("syn_attach").equals("1")
                && // Stop at conjunction in coordinated noun phrases
                            // if the left part already has its own noun
                            // (as in: "der 2. Mann und die 3. Frau", as opposed to
    ...
    /**
     * For a given token t, try to determine whether it is a definite or indefinite
     *  determiner.
     * @param t a token to verify.
     * @return "d" for definite determiner, "i" for indefinite determiner, and null
     *      if the token is not a determiner.
     */
