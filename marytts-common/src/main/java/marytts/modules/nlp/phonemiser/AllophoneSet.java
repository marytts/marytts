...
public class AllophoneSet {
    ...
    /**
     * Return the allophone set specified by the given filename. It will only be
     * loaded if it was not loaded before.
     *
     * @param filename
     *            filename
     * @return the allophone set, if one can be created. This method will never
     *         return null.
    ...
    /**
     * Determine whether the registry of previously loaded allophone sets
     * already contains an allophone set with the given identifier. If this
     * returns true, then a call to {@link #getAllophoneSetById(String)} with
     * the same identifier will return a non-null Allophone set.
     *
     * @param identifier
     *            the identifier of the allophone set to test.
     * @return true if the registry already contains the given allophone set,
     *         false otherwise.
     */
    ...
    /**
     * Get a previously loaded allophone set by its identifier. The method will
     * make no attempt to load the allophone set if it is not yet available.
     *
     * @param identifier
     ...
     * identified by the given identifier. It will only be loaded if it was not
     * loaded before.
     *
     * @param inStream
     *            an open stream from which the allophone set can be loaded. it
    ...
    private AllophoneSet(InputStream inputStream) throws MaryConfigurationException {
        ...
        NodeIterator ni = DomUtils.createNodeIterator(document, root, "vowel", "consonant", "silence",
                          "tone");
        Element a;
    ...
    public Allophone getAllophone(String ph) {
        ...
        if (allophone == null) {
            throw new IllegalArgumentException(String
                                               .format("Allophone `%s' could not be found in AllophoneSet `%s' (Locale: %s)", ph, name, locale));
        }
        return allophone;
    }
    ...
    /**
     * For the given feature name, get the list of all possible values that the
     * feature can take in this allophone set.
     *
     * @param featureName
     *            featureName
     * @throws IllegalArgumentException
     ...
     * primary and secondary stress, syllable boundaries, and spaces, will be
     * silently skipped.
     *
     * @param allophoneString
     *            the phonetic string to split
     * @return an array of Allophone objects corresponding to the string given
     *         as input
     * @throws IllegalArgumentException
    ...
    /**
     * Split allophone string into a list of allophone symbols. Include stress
     * markers (',) and syllable boundaries (-), skip space characters.
     *
     * @param allophoneString
     *            allophoneString
     * @throws IllegalArgumentException
     *             if the string contains illegal symbols.
     * @return a String containing allophones and stress markers / syllable
     *         boundaries, separated with spaces
     */
    ...
    /**
     * Split allophone string into a list of allophone symbols, preserving all
     * stress and syllable boundaries that may be present
     *
     * @param allophonesString
     *            allophonesString
     * @return a List of allophone Strings
     * @throws IllegalArgumentException
     *             if allophoneString contains a symbol for which no Allophone
     *             can be found
     */
    ...
    /**
     * Split allophone string into a list of allophone symbols. Include (or
     * ignore, depending on parameter 'includeStressAndSyllableMarkers') stress
     * markers (',), syllable boundaries (-). Ignores space characters.
     *
     * @param allophoneString
     * @param includeStressAndSyllableMarkers
     *            whether to skip stress markers and syllable boundaries. If
     *            true, will return each such marker as a separate string in the
     *            list.
     * @throws IllegalArgumentException
     *             if the string contains illegal symbols.
     * @return a list of allophone strings.
     */
    private List<String> splitIntoAllophoneList(String allophoneString,
            boolean includeStressAndSyllableMarkers) {
        ...
        for (int i = 0; i < allophoneString.length(); i++) {
            ...
            if (ph != null && allophones.containsKey(ph)) {
                ...
                // FIXME: temporarily handle digit suffix stress notation from
                // legacy LTS CARTs until these are rebuilt
                String stress = null;
                switch (ph) {
                case "1" :
    ...
    /**
     * Check whether the given allophone string has a correct syntax according
     * to this allophone set.
     *
     * @param allophoneString
     *            allophoneString
     ...
     * Principle (CSP)</i> from <blockquote>G.N. Clements (1990) "The role of
     * the sonority cycle in core syllabification." In: J. Kingston &amp; M.E.
     * Beckman (Eds.), <em>Papers in Laboratory Phonology I: Between the Grammar
     * and Physics of Speech</em>, Ch. 17, pp. 283-333, Cambridge University
     * Press.</blockquote>
     *
     * @param phoneString
     *            phoneString
     * @return a syllabified string; individual allophones are separated by
     *         spaces, and syllables, by dashes.
     * @throws IllegalArgumentException
     *             if the <b>phoneString</b> is empty or contains a symbol that
     *             satisfies none of the following conditions:
     *             <ol>
     *             <li>the symbol corresponds to an Allophone, or</li>
     *             <li>the symbol is a stress symbol (cf. {@link Stress}), or
     *             </li>
                    ...
                    if (allophone.getFeature("ctype").equals("r")) {
                        ...
                        if (iterator.previousIndex() > 1) {
                            ...
                            if (previousPhoneOrSyllable == currentSyllable) {
                                // the /6/ immediately follows the current
                                // syllable
                                if (!currentSyllable.getLastAllophone().isDiphthong()) {
                                    // the vowel immediately preceding the /6/
                                    // is not a diphthong
                                    appendR = true;
                                }
                            }
    ...
    /**
     * Helper class for OO syllabification. Wraps an ArrayList of Allophones and
     * has a Stress property.
     *
     * @author ingmar
     *
     */
    private class Syllable {
        ...
        /**
         * @return The Stress, if not {@link Stress.NONE NONE}, followed by the
         *         Allophones, all separated by spaces
         */
