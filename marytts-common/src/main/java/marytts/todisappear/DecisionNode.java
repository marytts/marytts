...
import marytts.todisappear.LeafNode.FeatureVectorLeafNode;
import marytts.todisappear.LeafNode.IntArrayLeafNode;
...
public abstract class DecisionNode extends Node {
    ...
    public void addDaughter(Node daughter) {
        if (lastDaughter > daughters.length - 1) {
            throw new RuntimeException("Can not add daughter number " + (lastDaughter + 1) +
                                       ", since node has only "
