...
public class ByteStringTranslator {
    /**
     * The maximum number of values that this translator can handle. This is the
     * highest possible number that we can still represent as an unsigned byte.
     */
    ...
    /**
     * Prepare a ByteStringTranslator to hold the given amount of data. After
     * calling this the {@link #getNumberOfValues()} will still return 0, since
     * there is no actual data yet.
     *
     * @param initialRange
     *            the number of values to expect
     * @throws IllegalArgumentException
     *             if initialRange is larger than the maximum number of values,
     *             {@link #MAXNUM}.
     */
    ...
    /**
     * Initialize a byte-string two-way translator, setting byte values
     * according to the position of strings in the array.
     *
     * @param strings
     *            a list of up to {@link #MAXNUM} strings to be represented by
     *            unique byte values.
     * @throws IllegalArgumentException
     *             if list of strings is longer than the maximum number of
     *             values, {@link #MAXNUM}.
     */
    ...
    /**
     * Associate the given (unsigned) byte with the given String. Values greater
     * than 127 can be used simply by casting an int to a byte:
    ...
    /**
     * Verify if the given string can be translated into a byte by this
     * translator.
     *
     * @param s
     *            s
    ...
    /**
     * Check if the given (unsigned) byte value is contained in the list. This
     * supports values between 0 and {@link #MAXNUM}, cast to byte:
     * <code>contains((byte)129)</code> will indicate if there is a String for
     * the 129'th byte value.
     *
     * @param b
     *            b
     ...
     * @param s
     *            s
     * @return the (unsigned) byte value associated to the given string. To cast
     *         this into an integer, use <code>value &amp; 0xFF</code>.
     * @throws IllegalArgumentException
    ...
    /**
     * Look up the (unsigned) byte in this translator. This supports values
     * between 0 and {@link #MAXNUM}, cast to byte: <code>get((byte)129)</code>
     * will get you the 129'th item in the string list.
     *
     * @param b
     *            b
