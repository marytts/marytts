...
public class SingularValueDecomposition implements java.io.Serializable {
	...
	public SingularValueDecomposition(Matrix Arg) {
		...
		/*
		 * Apparently the failing cases are only a proper subset of (m<n), so let's not throw error. Correct fix to come later? if
		 * (m<n) { throw new IllegalArgumentException("Jama SVD only works for m >= n"); }
		 */
		int nu = Math.min(m, n);
