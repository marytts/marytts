    ...
    throws Exception
    {
        ...
        for (LeafNode leaf : contourTree.getLeafNodes()) {
            ...
            for (int j=0; j<contourIDs.length; j++) {
                ...
                if (ArrayUtils.isZero(coeffs)) nZero++;
            }
            System.out.printf("Leaf %5d:%4d contours", numLeaves, contourIDs.length);
            if (nZero > 0) System.out.printf(" (%4d are zero)", nZero);
            System.out.println();
        }
        System.out.println("Tree has "+numLeaves+" leaves containing a total of "+totalSyls+" syllables (avg. "+(totalSyls/numLeaves)+ " syls per leaf)");
        for (LeafNode leaf : contourTree.getLeafNodes()) {
        }
        ...
        for (LeafNode leaf : contourTree.getLeafNodes()) {
            ...
            for (int i=0; i<n; i++) {
                ...
                if (rep == null) rep = new float[coeffs.length];
