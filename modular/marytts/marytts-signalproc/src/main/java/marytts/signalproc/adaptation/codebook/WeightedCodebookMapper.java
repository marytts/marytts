    ...
    public WeightedCodebookLsfMatch transform(double[] inputLsfs, WeightedCodebook codebook, boolean isVocalTractMatchUsingTargetCodebook, int[] preselectedIndices)
    {
            ...
            for (i=0; i<preselectedIndices.length; i++)
            {
                if (i>=mapperParams.numBestMatches)
                {
                    if (!isVocalTractMatchUsingTargetCodebook)
                        currentDist = DistanceComputer.getEuclideanDistance(inputLsfs, codebook.entries[preselectedIndices[i]].sourceItem.lsfs);
                    else
                        currentDist = DistanceComputer.getEuclideanDistance(inputLsfs, codebook.entries[preselectedIndices[i]].targetItem.lsfs);
                ...
                {
                    if (!isVocalTractMatchUsingTargetCodebook)
                        bestMatchDists[i] = DistanceComputer.getEuclideanDistance(inputLsfs, codebook.entries[preselectedIndices[i]].sourceItem.lsfs);
                    else
                        bestMatchDists[i] = DistanceComputer.getEuclideanDistance(inputLsfs, codebook.entries[preselectedIndices[i]].targetItem.lsfs);
        ...
        else if (mapperParams.distanceMeasure==WeightedCodebookMapperParams.LSF_INVERSE_HARMONIC_DISTANCE)
        {
            for (i=0; i<preselectedIndices.length; i++)
            {
                ...
                {
                    if (!isVocalTractMatchUsingTargetCodebook)
                        currentDist = DistanceComputer.getLsfInverseHarmonicDistance(inputLsfs, codebook.entries[preselectedIndices[i]].sourceItem.lsfs, mapperParams.freqRange);
                    else
                        currentDist = DistanceComputer.getLsfInverseHarmonicDistance(inputLsfs, codebook.entries[preselectedIndices[i]].targetItem.lsfs, mapperParams.freqRange);
                ...
                {
                    if (!isVocalTractMatchUsingTargetCodebook)
                        bestMatchDists[i] = DistanceComputer.getLsfInverseHarmonicDistance(inputLsfs, codebook.entries[preselectedIndices[i]].sourceItem.lsfs, mapperParams.freqRange);
                    else
                        bestMatchDists[i] = DistanceComputer.getLsfInverseHarmonicDistance(inputLsfs, codebook.entries[preselectedIndices[i]].targetItem.lsfs, mapperParams.freqRange);
        ...
        { 
            for (i=0; i<preselectedIndices.length; i++)
            {
                ...
                {
                    if (!isVocalTractMatchUsingTargetCodebook)
                        currentDist = DistanceComputer.getLsfInverseHarmonicDistanceSymmetric(inputLsfs, codebook.entries[preselectedIndices[i]].sourceItem.lsfs, mapperParams.alphaForSymmetric, mapperParams.freqRange);
                    else
                        currentDist = DistanceComputer.getLsfInverseHarmonicDistanceSymmetric(inputLsfs, codebook.entries[preselectedIndices[i]].targetItem.lsfs, mapperParams.alphaForSymmetric, mapperParams.freqRange);
                ...
                {
                    if (!isVocalTractMatchUsingTargetCodebook)
                        bestMatchDists[i] = DistanceComputer.getLsfInverseHarmonicDistanceSymmetric(inputLsfs, codebook.entries[preselectedIndices[i]].sourceItem.lsfs, mapperParams.alphaForSymmetric, mapperParams.freqRange);
                    else
                        bestMatchDists[i] = DistanceComputer.getLsfInverseHarmonicDistanceSymmetric(inputLsfs, codebook.entries[preselectedIndices[i]].targetItem.lsfs, mapperParams.alphaForSymmetric, mapperParams.freqRange);
        ...
        else if (mapperParams.distanceMeasure==WeightedCodebookMapperParams.LSF_MAHALANOBIS_DISTANCE)
        {
            ...
            for (i=0; i<preselectedIndices.length; i++)
            {
                ...
                {
                    if (!isVocalTractMatchUsingTargetCodebook)
                        currentDist = DistanceComputer.getMahalanobisDistance(inputLsfs, codebook.entries[preselectedIndices[i]].sourceItem.lsfs, inverseCovarianceMatrix);
                    else
                        currentDist = DistanceComputer.getMahalanobisDistance(inputLsfs, codebook.entries[preselectedIndices[i]].targetItem.lsfs, inverseCovarianceMatrix);
                ...
                {
                    if (!isVocalTractMatchUsingTargetCodebook)
                        bestMatchDists[i] = DistanceComputer.getMahalanobisDistance(inputLsfs, codebook.entries[preselectedIndices[i]].sourceItem.lsfs, inverseCovarianceMatrix);
                    else
                        bestMatchDists[i] = DistanceComputer.getMahalanobisDistance(inputLsfs, codebook.entries[preselectedIndices[i]].targetItem.lsfs, inverseCovarianceMatrix);
        ...
        else if (mapperParams.distanceMeasure==WeightedCodebookMapperParams.LSF_ABSOLUTE_VALUE_DISTANCE)
        {
            ...
            for (i=0; i<preselectedIndices.length; i++)
            {
                ...
                {
                    if (!isVocalTractMatchUsingTargetCodebook)
                        currentDist = DistanceComputer.getAbsoluteValueDistance(inputLsfs, codebook.entries[preselectedIndices[i]].sourceItem.lsfs);
                    else
                        currentDist = DistanceComputer.getAbsoluteValueDistance(inputLsfs, codebook.entries[preselectedIndices[i]].targetItem.lsfs);
                ...
                {
                    if (!isVocalTractMatchUsingTargetCodebook)
                        bestMatchDists[i] = DistanceComputer.getAbsoluteValueDistance(inputLsfs, codebook.entries[preselectedIndices[i]].sourceItem.lsfs);
                    else
                        bestMatchDists[i] = DistanceComputer.getAbsoluteValueDistance(inputLsfs, codebook.entries[preselectedIndices[i]].targetItem.lsfs);
        ...
        sortedIndicesOfBestMatchIndices = MathUtils.quickSort(bestMatchDists, 0, Math.min(mapperParams.numBestMatches, codebook.header.totalEntries)-1);
        ...
        weights = getWeights(bestMatchDists, Math.min(mapperParams.numBestMatches, codebook.header.totalEntries), mapperParams.weightingMethod, mapperParams.weightingSteepness);

        int j;
        String strBestIndices = "";
        String strWeights = "";
        
        returnedMatch = new WeightedCodebookLsfMatch(Math.min(mapperParams.numBestMatches, codebook.header.totalEntries), mapperParams.lpOrder);
        ...
        for (i=0; i<returnedMatch.totalMatches; i++)
        {
            ...
            {
                returnedMatch.entry.sourceItem.lsfs[j] += returnedMatch.weights[i]*codebook.entries[returnedMatch.indices[i]].sourceItem.lsfs[j];
                returnedMatch.entry.targetItem.lsfs[j] += returnedMatch.weights[i]*codebook.entries[returnedMatch.indices[i]].targetItem.lsfs[j];
            }
    ...
    public WeightedCodebookLsfMatch transformOld(double[] inputLsfs, WeightedCodebook codebook, boolean isVocalTractMatchUsingTargetCodebook)
    {
        ...
        else if (mapperParams.distanceMeasure==WeightedCodebookMapperParams.LSF_INVERSE_HARMONIC_DISTANCE)
        {
            for (i=0; i<codebook.header.totalEntries; i++)
            {
                ...
                {
                    if (!isVocalTractMatchUsingTargetCodebook)
                        currentDist = DistanceComputer.getLsfInverseHarmonicDistance(inputLsfs, codebook.entries[i].sourceItem.lsfs, mapperParams.freqRange);
                    else
                        currentDist = DistanceComputer.getLsfInverseHarmonicDistance(inputLsfs, codebook.entries[i].targetItem.lsfs, mapperParams.freqRange);
                ...
                {
                    if (!isVocalTractMatchUsingTargetCodebook)
                        bestMatchDists[i] = DistanceComputer.getLsfInverseHarmonicDistance(inputLsfs, codebook.entries[i].sourceItem.lsfs, mapperParams.freqRange);
                    else
                        bestMatchDists[i] = DistanceComputer.getLsfInverseHarmonicDistance(inputLsfs, codebook.entries[i].targetItem.lsfs, mapperParams.freqRange);
        ...
        { 
            for (i=0; i<codebook.header.totalEntries; i++)
            {
                ...
                {
                    if (!isVocalTractMatchUsingTargetCodebook)
                        currentDist = DistanceComputer.getLsfInverseHarmonicDistanceSymmetric(inputLsfs, codebook.entries[i].sourceItem.lsfs, mapperParams.alphaForSymmetric, mapperParams.freqRange);
                    else
                        currentDist = DistanceComputer.getLsfInverseHarmonicDistanceSymmetric(inputLsfs, codebook.entries[i].targetItem.lsfs, mapperParams.alphaForSymmetric, mapperParams.freqRange);
                ...
                {
                    if (!isVocalTractMatchUsingTargetCodebook)
                        bestMatchDists[i] = DistanceComputer.getLsfInverseHarmonicDistanceSymmetric(inputLsfs, codebook.entries[i].sourceItem.lsfs, mapperParams.alphaForSymmetric, mapperParams.freqRange);
                    else
                        bestMatchDists[i] = DistanceComputer.getLsfInverseHarmonicDistanceSymmetric(inputLsfs, codebook.entries[i].targetItem.lsfs, mapperParams.alphaForSymmetric, mapperParams.freqRange);
            ...
            for (i=0; i<codebook.header.totalEntries; i++)
            {
                if (i>=mapperParams.numBestMatches)
                {
                    if (!isVocalTractMatchUsingTargetCodebook)
                        ...
                        currentDist = DistanceComputer.getMahalanobisDistance(inputLsfs, codebook.entries[i].targetItem.lsfs, inverseCovarianceMatrix);
                ...
                else
                {
                    if (!isVocalTractMatchUsingTargetCodebook)
                        ...
                        bestMatchDists[i] = DistanceComputer.getMahalanobisDistance(inputLsfs, codebook.entries[i].targetItem.lsfs, inverseCovarianceMatrix);
            ...
            for (i=0; i<codebook.header.totalEntries; i++)
            {
                ...
                else
                {
                    if (!isVocalTractMatchUsingTargetCodebook)
                        ...
                        bestMatchDists[i] = DistanceComputer.getAbsoluteValueDistance(inputLsfs, codebook.entries[i].targetItem.lsfs);
