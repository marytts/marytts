...
public class NodeIterator<T extends Node> implements Iterator<T>
{
    ...
    {
        return (current == null
                || showDecisionNodes && current.isDecisionNode()
                || showLeafNodes && current.isLeafNode()
    ...
    private void nextNodeDepthFirst()
    {
        ...
        if (current.isDecisionNode()) {
            ...
            for (int i=0; i<dec.getNumberOfDaugthers(); i++) {
                ...
                if (unseenNode(dec.getDaughter(i))) return;
            }
    ...
    /**
     * Test whether the given node is unseen. If so, move current to it,
     * and remember it as a seen node.
     * @param candidate
     * @return
     */
