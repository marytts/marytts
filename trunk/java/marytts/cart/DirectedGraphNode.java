 ...
 * and that can have more than one mother.
 * Other than tree nodes, thus, directed graph nodes are not necessarily contained
 * in a strict tree structure; furthermore, each node can potentially carry data.
 * 
 * @author marc
 *
 */
public class DirectedGraphNode extends Node
{
    ...
    @Override public boolean isDirectedGraphNode()
    {
        return true;
    }
    ...
    {
        if (!(newNode == null ||
                newNode instanceof DirectedGraphNode ||
                newNode instanceof LeafNode)) {
    ...
    /**
     * Get a mother node of this node. DirectedGraphNodes can have
     * more than one node. 
     * 
    ...
    public int getNodeIndex()
    {
        ...
        if (firstMother != null) return motherToIndex.get(firstMother);
        return 0;
    }
     ...
     * @param aMother
     * @return
     * @throws IllegalArgumentException if mother is not a mother of this node.
     */
    ...
    {
        if (leafNode != null) leafNode.fillData(target, pos, len);
    }

    @Override
    public Object getAllData()
    {
        if (leafNode != null) return leafNode.getAllData();
        else if (decisionNode != null) return decisionNode.getAllData();
        return null;
    }
    ...
    {
        if (leafNode != null) return leafNode.getNumberOfData();
        else if (decisionNode != null) return decisionNode.getNumberOfData();
        return 0;
    }
    ...
    {
        if (decisionNode != null) return decisionNode.getNumberOfNodes();
        return 0;
    }
    ...
    public String getDecisionPath()
    {
        ...
        if (getMothers().size() == 0) ancestorInfo.append("null");
