				...
				for (l in lines) {
					...
					if (line.length > 0) {
						...
						if (line.indexOf("WAVE_FILE") != -1) {
							document.getElementById("AUDIO_OUT").selectedIndex = document
									.getElementById("AUDIO_OUT").length - 1;
						}
					}
				}
			} else {
...
/**
 * Update the current input text. Retrieves (1) the example text for the given
 * input type and locale, as well as (2) the example texts for the current
 * voice, if any. If there is a voice-specific example text and the input type
 * is TEXT, use the voice-specific example; else, use the type example. Replace
 * the content of input text only if replaceInput is true.
 */
	...
	if (replaceInput) {
		...
		xmlHttp.onreadystatechange = function() {
			if (xmlHttp.readyState == 4) {
				...
				if (replaceInput && !retrievingTypeExample
						&& !retrievingVoiceExample) {
					if (haveVoiceExample) {
						exampleChanged();
					} else {
...
/**
 * Set visibility of the modification checkbox so that it is shown if the
 * selected voice is of type "unitselection" and the selected output type is one
 * that requires AUDIO, and hidden otherwise.
 * 
 * @param {}
 *            voiceType "unitselection", "hmm", etc. will be filled if null
 * @param {}
 *            outputType "AUDIO", etc. will be filled if null
 * @return
 * @type
 */
function setModificationVisibility(voiceType, outputType) {
	...
	// TODO: as long as there is no InfoRequestHandler that returns the output
	// types which require AUDIO, just do:
	var outputTypesWithAudio = new Array("AUDIO", "REALISED_ACOUSTPARAMS",
			"REALISED_DURATIONS", "PRAAT_TEXTGRID");
...
function requestSynthesis() {
	...
	for (var i = 0; i < maryForm.length; i++) {
		...
		if (element.nodeName == "SELECT") {
			...
			if (element.id == "modification") {
				// if modification is visible and selected, set
				// OUTPUT_TYPE_PARAMS:
				if (document.getElementById("showHideModification").style.display != 'none'
						&& element.checked) {
					key = "OUTPUT_TYPE_PARAMS";
					value = "MODIFICATION";
				}
			} else if (element.id == "specifyTargetFeatures") {
				// if target features is visible and selected and not empty, set
				// OUTPUT_TYPE_PARAMS:
				if (document.getElementById("showHideTargetFeatures").style.display != 'none'
						&& element.checked) {
					var targetFeatures = document
							.getElementById("targetFeatureList").value;
	...
	if (outputType == "AUDIO") {
		...
		var mimeType = 'audio/'
				+ audioType.substring(0, audioType.indexOf('_')).toLowerCase();
		...
		if (!audioTag.canPlayType || audioTag.canPlayType(mimeType) == "no"
			...
			audioDestination.innerHTML = '<audio src="' + url
