...
public class MaryXMLToMbrola extends InternalModule {
	...
	public MaryData process(MaryData d) throws Exception {
		...
		NodeIterator it = ((DocumentTraversal) doc).createNodeIterator(doc, NodeFilter.SHOW_ELEMENT, new NameNodeFilter(
				new String[] { MaryXML.PHONE, MaryXML.BOUNDARY }), false);
		...
		if (currentVoice == null) {
			...
			logger.info("No default voice associated with data. Assuming locale " + locale + " default " + currentVoice.getName());
		}
		...
		while ((element = (Element) it.nextNode()) != null) {
			...
			if (v == null) {
				...
				if (newVoice != null && (buf.length() == 0 || newVoice != currentVoice)) {
	...
	/**
	 * Convert a given list of <ph> and <boundary> elements into the MBROLA .pho format.
	 * 
	 * @throws IllegalArgumentException
	 *             if one of the Elements in the List is not a <ph> or <boundary> element or if the voice is not an MBROLA voice.
	 */
	...
	/**
	 * Perform a power-on self test by processing some example input data. This module is actually tested as part of the
	 * MbrolaSynthesizer test, for which reason this method does nothing.
	 */
