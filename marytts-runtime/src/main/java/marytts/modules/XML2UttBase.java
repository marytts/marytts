...
/**
 * Convert a MaryXML DOM tree into FreeTTS utterances. This abstract base class is to provide the common part for all MaryXML to
...
public abstract class XML2UttBase extends InternalModule {
	 ...
	 * Depending on the data type, find the right information in the sentence and insert it into the utterance.
	 */
	protected void fillUtterance(Utterance utterance, Element sentence, boolean createWordRelation,
			boolean createSylStructRelation, boolean createTargetRelation) {
		...
		NodeIterator phraseIt = ((DocumentTraversal) doc).createNodeIterator(sentence, NodeFilter.SHOW_ELEMENT,
				new NameNodeFilter(MaryXML.PHRASE), false);
		...
		// The first token in a within-sentence <prosody> element receives the feature "prosody-start" with a non-null String
		// value,
		...
		while (topElement != null) {
			NodeIterator tokenIt = ((DocumentTraversal) doc).createNodeIterator(topElement, NodeFilter.SHOW_ELEMENT,
					new NameNodeFilter(new String[] { MaryXML.MTU, MaryXML.TOKEN, MaryXML.BOUNDARY }), false);
			Element element = null;
			while ((element = (Element) tokenIt.nextNode()) != null) {
				String elementText = addOneElement(utterance, element, createWordRelation, createSylStructRelation,
				...
				if (prosody != previousProsody) { // object equality is ok
					if (previousProsody != null && (prosody == null || !MaryDomUtils.isAncestor(previousProsody, prosody))) {
					...
					if (prosody != null && (previousProsody == null || !MaryDomUtils.isAncestor(prosody, previousProsody))) {
				...
				if (element.getTagName().equals(MaryXML.MTU)) {
					...
					Element mtu = element;
					while ((element = (Element) tokenIt.nextNode()) != null
							&& (element.getTagName().equals(MaryXML.TOKEN) || element.getTagName().equals(MaryXML.MTU))
		...
		if (createTargetRelation && createSylStructRelation) {
			...
			if (lastSegment != null && lastSegment.getFeatures().isPresent("end")) {
				...
				if (lastTarget != null)
					f0 = lastTarget.getFeatures().getFloat("f0");
				else
					f0 = 100;
	 ...
	 */
	protected String addOneElement(Utterance utterance, Element element, boolean createWordRelation,
			boolean createSylStructRelation, boolean createTargetRelation) {
		...
		if (element.getTagName().equals(MaryXML.BOUNDARY)) {
			...
			if (tokenText.matches("[.,!?:;]+") || pos.equals("punc")) { // it's a punctuation
				sentenceBuf.append(tokenText);
				Item lastToken = tokenRelation.getTail();
				if (lastToken != null) {
		...
		if (phoneList != null) {
			...
			tokenRelation.getTail().getFeatures().setObject("phones", phoneArray);
			phoneList = null;
		}
	...
	/**
	 * Append one item to the token relation, and return it. If tokenText is null, a "placeholder" token item will be created, in
	 * which no "name" feature is present, but to which other features can already be added. If the tail of tokenRelation is such
	 * a placeholder token, the tokenText will be set as that placeholder's name feature, and the ex-placeholder item will be
	 * returned. If the tail of tokenRelation is a placeholder and tokenText is null, no new placeholder will be created, i.e. it
	 * is not possible to create two subsequent placeholder items.
	 * 
	 * @param tokenRelation
	 * @param tokenText
	 * @return the new item
	...
	protected Item createSylStructure(Item wordItem, Element t, boolean createTargetRelation) {
		...
		while ((sylElement = (Element) sylIt.nextNode()) != null) {
			...
			NodeIterator segIt = ((DocumentTraversal) doc).createNodeIterator(sylElement, NodeFilter.SHOW_ELEMENT,
			...
			while ((segElement = (Element) segIt.nextNode()) != null) {
				...
				if (segElement.hasAttribute("d")) {
					try {
						...
						logger.debug("Cannot parse duration attribute `" + segElement.getAttribute("d") + "' as integer.", nfe);
					}
					segItem.getFeatures().setInt("mbr_dur", dur);
					totalDur += dur * 0.001f;
	...
	/**
	 * For a given element, extract essential settings defined by the closest ancestor prosody element and save them into the
	 * Utterance. Note that prosody settings outside of a paragraph or voice element, are not to be stored here, since they are
	 * dealt with elsewhere or are not relevant.
	 * 
	 * @param utterance
	 *            an utterance in which to save the prosody settings
	 * @param element
	 *            an element somewhere below the prosody element to convert.
	 */
	...
	/**
	 * Search backwards for <code>mark</code> elements that occur between the given element and the preceding (if any) element
	 * with the same tagname.
	 * 
	 * @param element
	 * @return A String containing a comma-separated list of all marks found between the given element and the preceding element
	 *         with the same name, or null if no mark was found.
	 */
	...
	protected String searchPrecedingMarks(Element element, String[] tagnames) {
		...
		TreeWalker tw = ((DocumentTraversal) doc).createTreeWalker(doc.getDocumentElement(), NodeFilter.SHOW_ELEMENT,
				new NameNodeFilter(searchTagnames), false);
		tw.setCurrentNode(element);
		Element prev;
		String mark = null;
		while ((prev = (Element) tw.previousNode()) != null && prev.getTagName().equals(MaryXML.MARK)) {
			if (mark == null)
	...
	/**
	 * Search forewards for <code>mark</code> elements that occur between the given element and the preceding (if any) element
	 * with the same tagname.
	 * 
	 * @param element
	 * @return A String containing a comma-separated list of all marks found between the given element and the preceding element
	...
	protected String searchFollowingMarks(Element element, String[] tagnames) {
		...
		Element prev;
		String mark = null;
		while ((prev = (Element) tw.nextNode()) != null && prev.getTagName().equals(MaryXML.MARK)) {
			if (mark == null)
				mark = prev.getAttribute("name");
			else
	...
	/**
	 * Converts a full phonetic string including stress markers into a list. Syllable boundaries, if present, will be ignored.
	 * Stress markers, if present, will lead to a "1" appended to the voice-specific versions of the vowels in the syllable.
	 * 
