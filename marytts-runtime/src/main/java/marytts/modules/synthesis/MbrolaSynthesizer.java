...
public class MbrolaSynthesizer implements WaveformSynthesizer {
	...
	public void startup() throws Exception {
		...
		try {
			maryxmlToMbrola = (MaryXMLToMbrola) ModuleRegistry.getModule(MaryXMLToMbrola.class);
		...
		if (System.getProperty("os.name").startsWith("Windows") && false) { // let's try without this, use cygwin binary
																			// instead...
		...
		if (!(obj instanceof MbrolaCaller)) {
			throw new ClassCastException("Class `" + mbrolaClass.getName() + "' is not an MbrolaCaller. Check property `"
					+ mbrolaCallerProperty + "' in configuration files");
		}
		...
		for (StringTokenizer st = new StringTokenizer(voiceNames); st.hasMoreTokens();) {
			...
			String path = MaryProperties.getFilename("voice." + voiceName + ".path", basePath + voiceName + File.separator
					+ voiceName);
			if (new File(path).exists()) {
				...
				Voice v = new MbrolaVoice(path, new String[] { voiceName }, locale, mbrolaAudioFormat(samplingRate), this,
						gender, topStart, topEnd, baseStart, baseEnd, voiceQualities, missingDiphones);
	...
	public synchronized void powerOnSelfTest() throws Error {
		...
		try {
			...
			if (example != null) {
				...
				mbrola.setAudioFileFormat(new AudioFileFormat(AudioFileFormat.Type.WAVE, Voice.AF22050, AudioSystem.NOT_SPECIFIED));
	...
	public AudioInputStream synthesize(List<Element> tokensAndBoundaries, Voice voice, String outputParams)
		...
		String pho = maryxmlToMbrola.convertToMbrola(phonesAndBoundaries, voice);
		...
		/*
		 * Vector audioInputStreams = new Vector(); long totalFrames = 0; StringTokenizer st = new StringTokenizer(pho, "#"); try
		 * { while (st.hasMoreTokens()) { AudioInputStream ais = mbrolaCaller.synthesiseOneSection(st.nextToken(), voice);
		 * audioInputStreams.add(ais); totalFrames += ais.getFrameLength(); } } catch (IOException ioe) { throw new
		 * SynthesisException("Cannot synthesise", ioe); } assert !audioInputStreams.isEmpty(); return
		 * MaryAudioUtils.createSingleAudioInputStream(audioInputStreams);
		 */
	...
	public static AudioFormat mbrolaAudioFormat(int samplingRate) {
		...
		} else if (System.getProperty("os.arch").equals("x86") || System.getProperty("os.arch").equals("i386")
		...
		return new AudioFormat(AudioFormat.Encoding.PCM_SIGNED, samplingRate, // samples per second
				16, // bits per sample
				1, // mono
