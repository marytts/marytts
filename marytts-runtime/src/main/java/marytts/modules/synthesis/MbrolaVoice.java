...
public class MbrolaVoice extends Voice {
	...
	public MbrolaVoice(String path, String[] nameArray, Locale locale, AudioFormat dbAudioFormat,
			WaveformSynthesizer synthesizer, Gender gender, int topStart, int topEnd, int baseStart, int baseEnd,
			String[] knownVoiceQualities, String missingDiphonesPath) throws MaryConfigurationException {
	...
	private void fillSampaMap() {
		...
		if (sampamapFilename != null) {
			...
			try {
				...
				logger.warn("Cannot open file '" + sampamapFilename + "' referenced in mary config file field voice." + getName()
						+ ".sampamapfile");
			}
	...
	private void addSampaMapEntry(String entry) throws IllegalArgumentException {
		...
		if (s2v) {
			if (sampa2voiceMap == null)
				sampa2voiceMap = new HashMap<String, String>();
			sampa2voiceMap.put(parts[0].trim(), parts[1].trim());
		}
		if (v2s) {
			if (voice2sampaMap == null)
				voice2sampaMap = new HashMap<String, String>();
			voice2sampaMap.put(parts[1].trim(), parts[0].trim());
		}
	}

	/**
	 * Converts a single phonetic symbol in the voice phonetic alphabet representation representation into its equivalent in MARY
	...
	/**
	 * Convert the SAMPA dialect used in MARY into the SAMPA version used in this voice. Allow for one-to-many translations,
	 * taking care of duration and f0 target adjustments.
	 * 
	 * @return a vector of MBROLAPhoneme objects realising this phone for this voice.
	 */
	...
	public Vector<MBROLAPhoneme> replaceDiphone(MBROLAPhoneme p1, MBROLAPhoneme p2) {
		...
		if (!solved) {
			...
			if (s1a != null) {
				...
				if (s2a != null && hasDiphone(s1 + "-" + s2a)) {
					...
					phones.add(p2);
					solved = true;
				} else if (s1a != null && s2a != null && hasDiphone(s1a + "-" + s2a)) {
					p1.setSymbol(s1a);
					p2.setSymbol(s2a);
					phones.add(p1);
					phones.add(p2);
		...
		// insert g before N?
		if (!solved && s2.equals("N") && hasDiphone(s1 + "-g") && hasDiphone("g-N")) {
			phones.add(p1);
		...
		if (!solved && (s1.equals("9") || s1.equals("O")) && hasDiphone(s1 + "-6") && hasDiphone("6-" + s2)) {
			phones.add(p1);
